...
interface SoftwareApp {
  ...
  status?: string
  heroImageUrl?: string
  screenshotUrls?: { caption: string; url: string }[]
  appStoreUrl?: string
  playStoreUrl?: string
}
...
const MODEL_SETTINGS: Record<string, {
  ...
  'prod-bonsai':    { cam:[-0.12,0.84,3.27], rot:[0.108,-1.032,-1.542], fov:41, exposure:0.65, ambient:0,   key:0.0, fill:0.8 },
          ...
          return (
            <a key={app._id} href={href} className="sw-card sw-card--img">
              ...
              <div className="sw-card-img" style={{background:'#000',display:'flex',alignItems:'center',justifyContent:'center'}}>
                {app.heroImageUrl ? (
                  isMobile ? (
                    /* 3-phone stack — screenshots via Sanity props */
                    (() => {
                      const shots = app.screenshotUrls || []
                      const byCaption: Record<string,string> = Object.fromEntries(shots.map((s: any) => [s.caption, s.url]))
                      const front = byCaption['Device List']     || app.heroImageUrl
                      const left  = byCaption['Parametric EQ']  || shots[4]?.url || app.heroImageUrl
                      const right = byCaption['X-Sense Auto EQ']|| shots[5]?.url || app.heroImageUrl
                      return (
                        <div style={{position:'relative',height:'188px',width:'100%',display:'flex',alignItems:'center',justifyContent:'center'}}>
                          <div style={{position:'absolute',right:'12%',top:'14px',width:'72px',height:'148px',border:'1.5px solid #222',borderRadius:'10px',background:'#080808',overflow:'hidden',boxShadow:'0 0 0 1px #111, 0 8px 24px rgba(0,0,0,0.9)',opacity:0.65,transform:'translateX(8px)',zIndex:1}}>
                            ...
                            {right && <img src={right} alt="X-Sense" style={{width:'100%',height:'100%',objectFit:'cover',objectPosition:'top'}}/>}
                          </div>
                          <div style={{position:'absolute',left:'12%',top:'14px',width:'72px',height:'148px',border:'1.5px solid #222',borderRadius:'10px',background:'#080808',overflow:'hidden',boxShadow:'0 0 0 1px #111, 0 8px 24px rgba(0,0,0,0.9)',opacity:0.65,transform:'translateX(-8px)',zIndex:1}}>
                            <div style={{position:'absolute',top:0,left:'50%',transform:'translateX(-50%)',width:'20px',height:'2.5px',background:'#1a1a1a',borderRadius:'0 0 3px 3px'}}/>
                            {left && <img src={left} alt="EQ" style={{width:'100%',height:'100%',objectFit:'cover',objectPosition:'top'}}/>}
                          </div>
                          <div style={{position:'relative',width:'82px',height:'168px',border:'1.5px solid #2a2a2a',borderRadius:'12px',background:'#0a0a0a',overflow:'hidden',boxShadow:'0 0 0 1px #181818, 0 12px 40px rgba(0,0,0,0.95)',zIndex:2}}>
                            ...
                            {front && <img src={front} alt={app.name} style={{width:'100%',height:'100%',objectFit:'cover',objectPosition:'top'}}/>}
                          </div>
                        </div>
                      )
                    })()
