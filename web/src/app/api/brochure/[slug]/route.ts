...
// Load all brand fonts as base64 @font-face — no Google Fonts network call needed
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{slug: string}> }) {
  ...
  try {
    ...
    const h = Buffer.from(`${P.productName}${P.powerRmsW}${P.depthMm}v5`).toString('base64').replace(/\W/g,'').slice(0,12)
    ...
    for (const acc of (P.accessories || [])) {
      ...
      if (n.includes('corner'))
        mountImages['corner'] = await imgB64(getRef(acc.lifestyleImage) || getRef(acc.heroImage), 550, acc.lifestyleImage?.crop, acc.lifestyleImage?.hotspot)
      else if (n.includes('floor') || n.includes('stand'))
        mountImages['floorstand'] = await imgB64(getRef(acc.lifestyleImage) || getRef(acc.heroImage), 550, acc.lifestyleImage?.crop, acc.lifestyleImage?.hotspot)
    }
    if (inWallRef) mountImages['in-wall'] = await imgB64(inWallRef, 550, inWallProduct?.lifestyle?.crop, inWallProduct?.lifestyle?.hotspot)

    // Fonts — all embedded, no network calls from Puppeteer
    const magmaFont = magmaWaveB64()
    const magmaCss  = magmaFont ? `@font-face{font-family:'MagmaWave';src:url('${magmaFont}') format('opentype');}` : ''
    ...
    const techIconsData = tech.map((badge: string) => ({ name: badge, img: techIconB64(badge) }))
    ...
    const colorTagsHtml = swatches.map((s:{name:string,hex:string}) =>
      `<div class="htag"><span class="htsw" style="background:${s.hex}"></span>${s.name}</div>`).join('')
    ...
    const galCells = [0,1,2,3].map(i =>
      galleryImgs[i] ? `<div class="gc"><img src="${galleryImgs[i]}" class="gi"></div>`
...
.cd{font-family:'DM Sans',Helvetica,sans-serif;font-size:10.5px;color:#7a776f;margin-top:12px;max-width:280px;line-height:1.7}
...
${coverPage ? `<div class="page" style="padding:0;margin:0"><img src="${coverPage}" style="width:100%;height:100%;object-fit:cover;display:block"></div>` : ''}
...
<div class="page">
  ...
  <div class="cl">
    ...
    <div class="tags">${colorTagsHtml}${ralTagHtml}${marineTagHtml}</div>
    <div class="cft">
...
${backPage ? `<div class="page" style="padding:0;margin:0;page-break-after:auto"><img src="${backPage}" style="width:100%;height:100%;object-fit:cover;display:block"></div>` : ''}
    ...
    // domcontentloaded — no waiting for external network (fonts are base64 embedded)
